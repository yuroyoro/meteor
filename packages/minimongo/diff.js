...
LocalCollection._diffQuery = function (old_results, new_results, observer) {
  ...
  // considered moved.
  //
  // This implementation is asymptotically good, but could be optimized
  // to streamline execution and use less memory (e.g. not have to build
  // data structures with an entry for every doc).
  ...
  var scan_to = function(old_j) {
    ...
    while (old_idx < old_j) {
      ...
      if (! is_in_new) {
        ...
        if (taken_set[old_idx]) {
          // already moved
          delete taken_set[old_idx];
        } else {
          // bump!
      ...
      if (old_doc_idx === undefined) {
        ...
        if (is_unmoved) {
          ...
          if (old_doc_idx >= old_idx) {
            ...
            // must take number of "taken" items into account;
            // this scan is unfortunate
            for(var i=old_idx; i<old_doc_idx; i++)
              if (taken_set[i])
                from_idx--;
            taken_set[old_doc_idx] = true;
          } else {
